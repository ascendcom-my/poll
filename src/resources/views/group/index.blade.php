<x-bigmom-auth::layout.main>
    ...
    <x-bigmom-auth::card class="pt-8">
        <x-bigmom-auth::button.yellow id="sort-save" data-route="{{ route('bigmom-poll.group.postSort', $group) }}">Save sorting</x-bigmom-auth::button.yellow>
        <ul id="sort-root" class="py-4 mt-8 bg-gray-200 rounded-xl" data-group="{{ $group }}">
            @foreach ($questions as $question)
            <li class="w-full" data-token="{{ $question->token }}">
            <x-bigmom-auth::card class="py-2">
